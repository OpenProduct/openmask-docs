...
In this tutorial, you deploy a single smart contract in one of TON networks. You have the option to run and review the implementation of the code with interact with the contract via web API.

This tutorial's steps are presented from a dApp developer perspective. You can complete most of these steps on a local machine running Linux, macOS, or Unix.
...
The following function creates an initial message. The message executes together with the deploy transaction, rirht after initialization.
...
To check smrt conract is working properly after deployment you may use [@openproduct/web-sdk](https://www.npmjs.com/package/@openproduct/web-sdk) package.
For example:
