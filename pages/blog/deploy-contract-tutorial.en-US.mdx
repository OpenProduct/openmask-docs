...
For deploying the process the contract developer has to provide initial data which would be stored in the smart contract cell and also the initial message with will be run with the deploy transaction.
...
My congratulation! You just deployed your smart contract to TON network.
